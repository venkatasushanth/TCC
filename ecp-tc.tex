...
\coadvisor[Prof.~Dr]{Zimpeck}{Alexandra L.}
...
FinFET technology is built on a Silicon on Insulator (SOI) substrate with a conducting channel that rises above the level of the insulator, creating a thin silicon structure, the gate as shown in Figure \ref{fig:Fig1}. The channel being surrounded from three dimensions by the gate results in a superior control, reduced short-channel effect (SCE) and eliminated random dopant fluctuation (RDF) effect due to the fully depleted channel that causes less sensitivity to process variations \cite{taur2013fundamentals}. 
...
\label{wff}
\legend{Source: \cite{meinhardt2014impact}}
\end{figure}
...
\label{fig:layers}
\legend{Source: \cite{clark2016asap7}}
\end{figure}
